...
with tabs[0]:
    ...
    if save and title.strip(): add_handover_note(store,title,dept,owner,pri,detail,amap.get(asset,""),due); persist(store); st.rerun()
...
with tabs[1]:
    ...
    if create and title.strip(): create_work_order(store,title,dept,amap.get(asset,""),typ,pri,owner,req,due,scope,safety,est,pd,mmap.get(srcm,""),actmap.get(srca,"")); persist(store); st.rerun()
    ...
    if womap:
        ...
        if save: update_work_order(store,uid,status=status,owner=owner,actual_hours=actual,actual_downtime_hours=down,parts_required=parts,completion_notes=notes); persist(store); st.rerun()
...
with tabs[2]:
    ...
    if create and title.strip(): create_shutdown(store,title,start,end,dept,coord,[amap[x] for x in assets],scope,pre,parts,contractor,safety,restart); persist(store); st.rerun()
    ...
    if smap and womap:
        ...
        if st.button("Save Shutdown Links / Status"): s["linked_work_order_uuids"]=[womap[x] for x in linked]; s["status"]=status; persist(store); st.rerun()
...
with tabs[3]:
    ...
    if amap:
        ...
        if save and metric.strip(): add_condition_reading(store,amap[asset],metric,value,unit,rd,warning,critical,direction,source,notes); persist(store); st.rerun()
...
with tabs[4]:
    ...
    if amap:
        ...
        if save: save_risk_assessment(store,uid,safety,prod,quality,repair,likelihood,notes); persist(store); st.rerun()
...
with tabs[5]:
    ...
    if recs:
        ...
        if st.button("Create Recommended RCA"): create_rca_from_breakdown(store,choices[pick],owner); persist(store); st.rerun()
