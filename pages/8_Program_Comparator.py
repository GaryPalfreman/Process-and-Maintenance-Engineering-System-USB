...
if is_cnc:
            ...
            for index, change in enumerate(visible, start=1):
                ...
                with st.expander(f"{index}. {change['status'].upper()} — old line {line1} ↔ new line {line2} — {labels}", expanded=index <= 5):
                    ...
                    if change["status"] == "modified":
                        ...
                        if change["details"]:
                            st.dataframe(change["details"], use_container_width=True, hide_index=True, column_order=["category", "address", "file1", "file2"])

    with tab_summary:
        counts = result["category_counts"]
        if counts:
            rows = [{"Change category": category, "Occurrences": count} for category, count in sorted(counts.items(), key=lambda x: (-x[1], x[0]))]
        ...
        for change in changes:
            if change["details"]:
                for detail in change["details"]:
                    writer.writerow([change["status"], change["file1_line"] or "", change["file2_line"] or "", " | ".join(change["categories"]), detail["address"], detail["file1"], detail["file2"], change["file1"], change["file2"]])
            else:
                writer.writerow([change["status"], change["file1_line"] or "", change["file2_line"] or "", " | ".join(change["categories"]), "", "", "", change["file1"], change["file2"]])
        report_bytes = report_buffer.getvalue().encode("utf-8")
        report_name = f"{file1.name.rsplit('.', 1)[0]}_vs_{file2.name.rsplit('.', 1)[0]}_comparison.csv"
    ...
    with tab_compare:
        ...
        for diff in diffs:
            report_text.extend([f"Line {diff['line']} ({diff['similarity']}%)", f"OLD: {diff['file1']}", f"NEW: {diff['file2']}", ""])
        report_bytes = "\n".join(report_text).encode("utf-8")
        report_name = f"{file1.name.rsplit('.', 1)[0]}_vs_{file2.name.rsplit('.', 1)[0]}_comparison.txt"

with tab_files:
    c1, c2 = st.columns(2)
    with c1:
        st.markdown(f"**Old/reference — {file1.name}**")
        st.code(content1, language=None, line_numbers=True)
    with c2:
        st.markdown(f"**New/revised — {file2.name}**")
...
with tab_general:
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.markdown(f"**Only in old/reference ({len(general['only_file1'])})**")
        st.code("\n".join(general["only_file1"]) or "No unique lines", language=None)
    with c2:
        st.markdown(f"**Only in new/revised ({len(general['only_file2'])})**")
        st.code("\n".join(general["only_file2"]) or "No unique lines", language=None)

with tab_merge:
    if is_cnc:
        st.warning("CNC merge remains line-based. Review merged output carefully before any machine use.")
    prefer = st.radio("When corresponding lines differ, which file should win?", ["New / revised", "Old / reference"], horizontal=True)
    merged = merge_files(content1, content2, prefer="file2" if prefer == "New / revised" else "file1")
